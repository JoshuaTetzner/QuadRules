...
function checkindex(node::Vector, lastnodes::Vector)
    quad = argmin(lastnodes)
    if isapprox((node[1] + node[2]),0, atol=1e-14)
        return true, lastnodes
    else
        if quad == 1
            if minimum(node) > 0
                lastnodes[quad] += 1
...
function balancednonsymmetricquad(
    nodes::Matrix{T},
    weights::Vector{T},
    order::Int
